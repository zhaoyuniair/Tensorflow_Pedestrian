...
from SuperDataset import SuperDatasetManager
from SuperDataset import SuperDataset


def createCalPedCategoryDict():
    # t_list = ['person', 'background']
    t_list = ['background', 'person']
    t_dict = {}
    cnt = 0
    for key in t_list:
        t_dict[key] = cnt
...
class CalPedManager(SuperDatasetManager):
    ...
    def __init__(self,
        ...
        super(CalPedManager, self).__init__()
        ...
        self.maxImageSize = 0
        self.numCate = 0
        self.CalPedCateDict = createCalPedCategoryDict()
    ...
    def readDataset(self, txtPath):
        ...
        :return: None
        """
        CalPedCateDict = self.CalPedCateDict
        numCate = len(CalPedCateDict)
        self.numCate = numCate
        # Obtain the directions of images and annotations
        fdir = txtPath
        setName = 'xxx'
        while re.match(r'CalPed.{4}', setName) is None:
        ...
        antList = []
        imageSize = []
        for fileName in fileNameList[0:10]:
        ...
        if self.enableMemSave:
            ...
            imgsShape = [num, maxImageSize[1], maxImageSize[0], maxImageSize[2]]
            ...
            self.labels = np.zeros(shape=[num, self.bboxMaxNum, numCate],
                                   dtype=np.float32)
            # self.images_mean_list = np.zeros(shape=[num, 3],
            #                             dtype=np.float32)
            ...
            for i in range(0, num):
                ...
                #     self.readOneDataPoint(t_imgPath, t_antPath, CalPedCateDict)
                img, imgPos, bbox, labels = \
                    self.readOneDataPointWithoutBox(t_imgPath, t_antPath, CalPedCateDict)
                self.images[i, :] = img
                self.imagesPosition[i, :] = imgPos
                self.bbox[i, :] = bbox
                self.labels[i, :] = labels
                # temp_mean = np.mean(img, 0)
                # self.images_mean_list[i, :] = np.mean(np.mean(img, 0), 0)
            # self.images_mean = np.mean(self.images_mean_list, 0)

                # end_time = time.clock()
                # print 'loading %d, using: %f s;' %(i, (end_time-start_time))
    ...
    def readOneDataPointWithoutBox(self, t_imgPath, t_antPath, CalPedCateDict):
        """

        :param t_imgPath:
        :param t_antPath:
        :param CalPedCateDict:
        :return:
        """
        # Create memory
        maxImageSize = self.maxImageSize
        image = np.zeros(shape=[maxImageSize[1], maxImageSize[0], maxImageSize[2]],
                         dtype=np.uint8)
        ...
        cateIndex = CalPedCateDict[objs[0]['name']]
        labels[0, cateIndex] = 1.0
        imgPos = [0, 0, 64, 128]
        bboxes = [0, 0, 64, 128]

        return image, imgPos, bboxes, labels


    def readOneDataPoint(self, t_imgPath, t_antPath, CalPedCateDict):
        """
        Function: Read a data point (image, annotation)
        :param t_imgPath: the path of image
        :param t_antPath:  the path of annotation
        :param CalPedCateDict: the dictionary of categoties
        ...
        lux = maxImageSize[0] / 2 - size[0] / 2
        luy = maxImageSize[1] / 2 - size[1] / 2
        rdx = lux + size[0]
        rdy = luy + size[1]
        ...
        if len(posIndex) > 0:
            for j in range(0, minNum):
                ...
                cateIndex = CalPedCateDict[objs[posIndex[j]]['name']]
        ...
        if minNum < self.bboxMaxNum:
            ...
            for j in range(0, len(objs)):
                ...
                if not objsUsedFlag[j]:
                    ...
                    cateIndex = CalPedCateDict[objs[j]['name']]
...
class Config(object):
    ...
    def __init__(self,
                 ...
                 isUseAllData=True,
                 isTheLargestObj=True,
                 specificClassName='person',
                 datasetDir='/home/jiwu/Database/Caltech-dataset/CalPedProp'):
        """
...
class CalPedDataset(SuperDataset):

    def __init__(self, config=Config()):
        super(CalPedDataset, self).__init__()
    ...
    def readDataset(self, isTrain=True, isTest=True):
        ...
        start = time.clock()
        if isTest:
            print 'Reading CalPed-dataset testset ...'
            self.testset.readDataset(testListPath)
        if isTrain:
            print 'Reading CalPed-dataset trainset ...'
            self.trainset.readDataset(trainListPath)
        end = time.clock()
        print 'Read CalPed-dataset completely, cost time %f seconds' \
              % (end - start)
...
def main():
    print sys.argv
    datapath = '/home/jiwu/Database/Caltech-dataset/CalPedProp/ImageSets/trainset.txt'
    ...
    config = Config(
        ...
        specificClassName='',
        datasetDir='/home/jiwu/Database/Caltech-dataset/CalPedProp')
    cal = CalPedDataset(config=config)
    cal.readDataset()

    print 'trainingSampleNum = ', cal.trainset.getSampleNum()
    print 'testingSampleNum = ', cal.testset.getSampleNum()
    for i in range(0, 1000):
        images, bbox, labels = cal.trainset.getNextBatchWithLabels()
        print cal.trainset.epochCount
        print images.shape
        print bbox.shape
        print labels.shape
        print labels
        for ii in range(0, images.shape[0]):
            cal.trainset.showBbox(images, bbox, ii)
